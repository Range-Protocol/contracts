...
describe("RangeProtocolVault: mint-burn test", () => {
  before(async () => {
    ...
    nativeTokenSupportLib = (await (
      await ethers.getContractFactory("NativeTokenSupport")
    ).deploy()) as NativeTokenSupport;
  ...
  it("should mint with ERC20 tokens", async () => {
    ...
    await vault.mint(mintAmount, false, [amount0, amount1]);
    console.log("*** AFTER ***");
    console.log(
      "token0: ",
      ethers.utils.formatEther(await token0.balanceOf(user.address))
  ...
  it.skip("should revert sending native asset when minting with ERC20 tokens", async () => {
    ...
    await expect(
      vault.mint(mintAmount, false, [amount0, amount1],{
  ...
  it("should mint with native asset", async () => {
    ...
    const { cumulativeGasUsed, effectiveGasPrice } = await (
      await vault.mint(mintAmount, true, [amount0, amount1],{
        value: nativeAmount,
      })
    ).wait();
    console.log(
