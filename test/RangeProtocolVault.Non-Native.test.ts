...
describe("RangeProtocolVault::Non-Native", () => {
  before(async () => {
  ...
  it("should not allow minting with zero mint amount", async () => {
    const mintAmount = 0;
    await expect(
      vault.mint(mintAmount, false, [0, 0])
  ...
  it("should not mint when contract is paused", async () => {
    ...
    await expect(vault.mint(mintAmount, false, [0, 0])).to.be.revertedWith(
      "Pausable: paused"
    );
  ...
  it("should mint with zero totalSupply of vault shares", async () => {
    ...
    await expect(vault.mint(mintAmount, false, [_amount0, _amount1]))
  ...
  it.skip("should not accept native tokens when deposit is non native", async () => {
    ...
    await expect(
      vault.mint(mintAmount, false, [_amount0, _amount1], { value: _amount1 })
  ...
  it.skip("should not mint when less than required native token is supplied", async () => {
    ...
    await token0.approve(vault.address, 0);
    await token1.approve(vault.address, 0);

    await expect(
      vault.mint(mintAmount, true, [_amount0, _amount1], {
  ...
  it("should mint with non-native tokens", async () => {
    ...
    await vault.mint(mintAmount, true, [_amount0, _amount1], {
      value: _amount1,
    });
    const userBalance0After = await token0.balanceOf(manager.address);
    const userBalance1After = await token1.balanceOf(manager.address);
    const userShareAfter = await vault.balanceOf(manager.address);
    const totalSupplyAfter = await vault.totalSupply();
    expect(userBalance0After).to.be.equal(userBalance0Before.sub(_amount0));
    expect(userBalance1After).to.be.equal(userBalance1Before.sub(_amount1));
  ...
  it("should not burn non existing vault shares", async () => {
    ...
    await expect(
      vault.connect(user2).burn(burnAmount, false, [0, 0])
  ...
  it("should burn vault shares", async () => {
    ...
    await expect(vault.burn(burnAmount, false, [minAmount0, minAmount1]))
  ...
  it.skip("should burn vault shares and receive non-native tokens", async () => {
    const {
      mintAmount,
      // eslint-disable-next-line @typescript-eslint/naming-convention
      amount0: _amount0,
    ...
    await vault.burn(burnAmount, true, [minAmount0, minAmount1]);
  });
  ...
  it("should not add liquidity when total supply is zero and vault is out of the pool", async () => {
    ...
    await vault.mint(mintAmount, false, [_amount0, _amount1]);
    ...
    await expect(
      vault.mint(mintAmount, false, [0, 0])
    ).to.be.revertedWithCustomError(vault, "MintNotAllowed");
  });
  ...
  describe("Remove Liquidity", () => {
    ...
    it("should burn vault shares when liquidity is removed", async () => {
      ...
      const {amount0: minAmount0, amount1: minAmount1} = await vault.getUnderlyingBalancesByShare(vaultShares);
      await expect(vault.burn(vaultShares, false, [minAmount0, minAmount1])).not.to.emit(
        vault,
        "FeesEarned"
      );
      expect(await token0.balanceOf(manager.address)).to.be.equal(
