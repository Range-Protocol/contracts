...
describe.only("RangeProtocolVault::Non-Native", () => {
    before(async () => {
    ...
    it("should mint with non-native tokens", async () => {
        ...
        expect(userBalance0After).to.be.equal(
            userBalance0Before.sub(_amount0)
        );
        expect(userBalance1After).to.be.equal(
            userBalance1Before.sub(_amount1)
        );
