...
import {IButterFactory} from "./interfaces/butter/IButterFactory.sol";
...
contract RangeProtocolFactory is IRangeProtocolFactory, Ownable {
    ...
    constructor(address _butterFactory, address _deployer) Ownable() {
        factory = _butterFactory;
        deployer = _deployer;
    ...
    // @param tokenA one of the tokens in the butter pair
    // @param tokenB the other token in the butter pair
    // @param fee fee tier of the butter pair
    ...
    function createVault(
        ...
        if (msg.sender != deployer) revert FactoryErrors.OnlyDeployerCanDeployVault();
        address pool = IButterFactory(factory).getPool(tokenA, tokenB, fee);
    ...
    /**
     * @notice allows owner to change deployer address.
    ...
    function _createVault(
        ...
        int24 tickSpacing = IButterFactory(factory).feeAmountTickSpacing(fee);
        vault = address(
            new ERC1967Proxy(
                implementation,
