...
import {IFusionXV3Pool} from "./fusionX/interfaces/IFusionXV3Pool.sol";

import {TickMath} from "./fusionX/TickMath.sol";
import {LiquidityAmounts} from "./fusionX/LiquidityAmounts.sol";
import {FullMath} from "./fusionX/FullMath.sol";
 ...
 * @notice RangeProtocolVault is fungible vault shares contract that accepts fusionX pool tokens for liquidity
 * provision to the corresponding fusionX v3 pool. This contract is configurable to work with any fusionX v3
 ...
 * active position in the fusionX v3 pool) and burning involves removing liquidity from the fusionX v3 pool along with
 * the vault's fee.
 *
 * The manager of the contract can remove liquidity from fusionX v3 pool and deposit into a newer take range to maximise
 * the profit by keeping liquidity out of the pool under high volatility periods.
 *
 * Part of the fee earned from fusionX v3 position is paid to manager as performance fee and fee is charged on the LP's
    ...
    RangeProtocolVaultStorage
{
     ...
     * @param _pool address of the fusionX v3 pool associated with vault
     * @param _tickSpacing tick spacing of the fusionX pool
    ...
    /// @notice fusionXV3MintCallback fusionX V3 callback fn, called back on pool.mint
    ...
    /// @notice fusionXV3SwapCallback fusionX v3 callback fn, called back on pool.swap
    ...
    /**
     * @notice mint mints range vault shares, fractional shares of a fusionX V3 position/strategy
    ...
    /**
     * @notice burn burns range vault shares (shares of a fusionX V3 position) and receive underlying
    ...
    /**
     * @notice removeLiquidity removes liquidity from fusionX pool and receives underlying tokens
    ...
    /**
     * @dev Mars@RangeProtocol
     * @notice addLiquidity allows manager to add liquidity into fusionX pool into newer tick ranges.
    ...
    /**
     * @dev pullFeeFromPool pulls accrued fee from fusionX v3 pool that position has accrued since
     * last collection.
     */
    ...
    /**
     * @notice getPositionID returns the position id of the vault in fusionX pool
     * @return positionID position id of the vault in fusionX pool
     */
     ...
     * includes current liquidity invested in fusionX position, current fees earned
     ...
     * @param feeGrowthInsideLast last time the fee was realized for the vault in fusionX pool
     */
    ...
    /**
     * @notice _applyPerformanceFee applies the performance fee to the fees earned from fusionX v3 pool.
    ...
    /**
     * @notice _netPerformanceFees computes the fee share for manager as performance fee from the fee earned from fusionX v3 pool.
     * @param rawFee0 fee earned in token0 from fusionX v3 pool.
     * @param rawFee1 fee earned in token1 from fusionX v3 pool.
