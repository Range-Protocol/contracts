...
import {IAgniPool} from "./interfaces/agni/IAgniPool.sol";

import {TickMath} from "./agni/TickMath.sol";
import {LiquidityAmounts} from "./agni/LiquidityAmounts.sol";
import {FullMath} from "./agni/FullMath.sol";
 ...
 * @notice RangeProtocolVault is fungible vault shares contract that accepts agni pool tokens for liquidity
 * provision to the corresponding agni v3 pool. This contract is configurable to work with any agni v3
 ...
 * active position in the agni v3 pool) and burning involves removing liquidity from the agni v3 pool along with
 * the vault's fee.
 *
 * The manager of the contract can remove liquidity from agni v3 pool and deposit into a newer take range to maximise
 * the profit by keeping liquidity out of the pool under high volatility periods.
 *
 * Part of the fee earned from agni v3 position is paid to manager as performance fee and fee is charged on the LP's
    ...
    RangeProtocolVaultStorage
{
     ...
     * @param _pool address of the agni v3 pool associated with vault
     * @param _tickSpacing tick spacing of the agni pool
    ...
    /**
     * @notice mint mints range vault shares, fractional shares of a Agni V3 position/strategy
    ...
    /**
     * @notice burn burns range vault shares (shares of a Agni V3 position) and receive underlying
    ...
    /**
     * @notice removeLiquidity removes liquidity from agni pool and receives underlying tokens
    ...
    /**
     * @dev Mars@RangeProtocol
     * @notice addLiquidity allows manager to add liquidity into agni pool into newer tick ranges.
    ...
    /**
     * @dev pullFeeFromPool pulls accrued fee from agni v3 pool that position has accrued since
     * last collection.
     */
    ...
    /**
     * @notice getPositionID returns the position id of the vault in agni pool
     * @return positionID position id of the vault in agni pool
     */
     ...
     * includes current liquidity invested in agni position, current fees earned
    ...
    /**
     * @notice _withdraw internal function to withdraw liquidity from agni pool
     * @param liquidity liquidity to remove from the agni pool
     */
    function _withdraw(
        uint128 liquidity
     ...
     * @param feeGrowthInsideLast last time the fee was realized for the vault in agni pool
     */
    ...
    /**
     * @notice _applyPerformanceFee applies the performance fee to the fees earned from agni v3 pool.
    ...
    /**
     * @notice _netPerformanceFees computes the fee share for manager as performance fee from the fee earned from agni v3 pool.
     * @param rawFee0 fee earned in token0 from agni v3 pool.
     * @param rawFee1 fee earned in token1 from agni v3 pool.
