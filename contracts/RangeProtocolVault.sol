...
import {IButterPool} from "./interfaces/butter/IButterPool.sol";

import {TickMath} from "./butter/TickMath.sol";
import {LiquidityAmounts} from "./butter/LiquidityAmounts.sol";
import {FullMath} from "./butter/FullMath.sol";
 ...
 * @notice RangeProtocolVault is fungible vault shares contract that accepts butter pool tokens for liquidity
 * provision to the corresponding butter pool. This contract is configurable to work with any butter
 ...
 * active position in the butter pool) and burning involves removing liquidity from the butter pool along with
 * the vault's fee.
 *
 * The manager of the contract can remove liquidity from butter pool and deposit into a newer take range to maximise
 * the profit by keeping liquidity out of the pool under high volatility periods.
 *
 * Part of the fee earned from butter position is paid to manager as performance fee and fee is charged on the LP's
    ...
    RangeProtocolVaultStorage
{
    ...
    /// Performance fee cannot be set more than 20% of the fee earned from butter pool.
     ...
     * @param _pool address of the butter pool associated with vault
     * @param _tickSpacing tick spacing of the butter pool
    ...
    /// @notice butterMintCallback Butter callback fn, called back on pool.mint
    ...
    /// @notice butterSwapCallback Butter callback fn, called back on pool.swap
    ...
    /**
     * @notice mint mints range vault shares, fractional shares of a Butter position/strategy
    ...
    /**
     * @notice burn burns range vault shares (shares of a Butter position) and receive underlying
    ...
    /**
     * @notice removeLiquidity removes liquidity from butter pool and receives underlying tokens
    ...
    /**
     * @dev Mars@RangeProtocol
     * @notice addLiquidity allows manager to add liquidity into butter pool into newer tick ranges.
    ...
    /**
     * @dev pullFeeFromPool pulls accrued fee from butter pool that position has accrued since
     * last collection.
     */
    ...
    /**
     * @notice getPositionID returns the position id of the vault in butter pool
     * @return positionID position id of the vault in butter pool
     */
     ...
     * includes current liquidity invested in butter position, current fees earned
    ...
    /**
     * @notice _withdraw internal function to withdraw liquidity from butter pool
     * @param liquidity liquidity to remove from the butter pool
     */
    function _withdraw(
        uint128 liquidity
     ...
     * @param feeGrowthInsideLast last time the fee was realized for the vault in butter pool
     */
    ...
    /**
     * @notice _applyPerformanceFee applies the performance fee to the fees earned from butter pool.
    ...
    /**
     * @notice _netPerformanceFees computes the fee share for manager as performance fee from the fee earned from butter pool.
     * @param rawFee0 fee earned in token0 from butter pool.
     * @param rawFee1 fee earned in token1 from butter pool.
